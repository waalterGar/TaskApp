...
function SessionCard({ session, athlete}) {
  const { deleteTask, toggleTask } = useSessions();
  const navigate = useNavigate();
    console.log("session card: ",session);
  return (
    <div className="bg-zinc-700 text-white rounded-md p-4 flex cursor-pointer" onClick={() => navigate(`/athlete/${athlete.id}/sessions/${session.id_training_session}/executions/`)}>
